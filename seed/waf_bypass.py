...
class Bypass:
    # 주석처리기법
    def wafBypassComment(self,outputName, inputFile):
    ...
    # 대소문자 혼용 & 변경 기법
    def wafBypassCase(self,outputName, inputFile):
        outputFile = open(outputName, 'w')
        while True:
            line = inputFile.readline()
            if not line: break
    ...
    # 키워드 대체기법
    def wafBypassKeyword(self,outputName, inputFile):
        outputFile = open(outputName, 'w')
        while True:
            line = inputFile.readline()
            if not line: break
    ...
    # 특수 키워드 대체 기법
    def wafBypassFilteringKey(self, outputName, inputFile):
        outputFile = open(outputName, 'w')
